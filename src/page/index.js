    ...
    <div className={style.main}>
      ...
      <section className={style.sidebar}>
        ...
        <div className={style.chatlist}>
          {
            user ? user.map(data => 
            <Card name={data.fullname} selected={data.id === receiver_id} photo={data.photo} key={data.id} lastTime={data.lastTime} lastMessage={data.lastMessage} onClick={() => setReceiver_id(data.id)} />) : <div>come start to chat user</div>
          }
        </div>
      </section>
      {/* content chat */}
